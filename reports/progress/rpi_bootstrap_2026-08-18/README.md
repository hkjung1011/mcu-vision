# Raspberry Pi SBC 1-class 객체 검출 학습 중간 보고서 — 2026-08-18

> **INTERIM_PROGRESS · NOT_FORMAL_RELEASE · NOT_INDEPENDENT_TESTED**
>
> 이 결과는 Raspberry Pi SBC **1-class 파이프라인 검증** 전용입니다. STM32, Raspberry Pi Pico,
> loose SMD, top-mark OCR 또는 SMD/STM32 6-class 모델의 결과가 아닙니다.

의도된 용도, 금지된 용도, 전처리와 잔여 위험은
[한국어 모델 카드](../../../weights/progress/rpi_bootstrap_2026-08-18/MODEL_CARD.ko.md)에 별도로
정리했습니다.
...
| Run | 상태 | 평가 근거 | AP50-95 | AP50 | P/R/F1 @ conf 0.25 | FP16 E2E p50/p95 | FPS |
|---|---:|---|---:|---:|---:|---:|---:|
| YOLO11m seed42 | COMPLETE 100/100 | 공통 COCO 검증 세트 | 1.000000 | 1.000000 | 1/1/1 | 15.68/16.96 ms | 64.13 |
| YOLO11m seed43 | COMPLETE 100/100 | 공통 COCO 검증 세트 | 1.000000 | 1.000000 | 1/1/1 | 11.52/12.46 ms | 84.92 |
| YOLOX-S seed42 | COMPLETE 100/100 | 공통 COCO 검증 세트 | 0.979373 | 1.000000 | 1/1/1 | 17.76/20.05 ms | 56.43 |
| YOLOX-S seed43 | INTERRUPTED 70/100 | native 검증 세트 | best 0.857410 (epoch 34) | 1.000000 | 미산출 | 미산출 | 미산출 |

중단 run의 epoch 70 native AP50-95는 `0.834207`, AR100은 `0.853333`입니다. 이 값은
완료 실행의 공통 최종 evaluator 결과가 아니므로 같은 열에서 모델 우열을 판단해서는 안 됩니다.
...
- 학습: 공식 COCO pretrained `yolo11m.pt`에서 모든 trainable weight fine-tuning,
  SGD, AMP, batch 8, 100 epochs
- 현재 검증 세트에서의 관찰값: 높은 box localization AP와 비교적 짧은 실측 지연 시간
- 주의: seed42/43 지연 시간 차이가 커서 노트북의 열·전력·background load 영향을 포함합니다.
...
- 학습: 공식 COCO pretrained YOLOX-S에서 class-specific output convolution 6개는 재초기화하고
  호환 tensor는 전이, SGD Nesterov, AMP, batch 8, 100 epochs 계획
- 현재 검증 세트에서의 관찰값: YOLO11m보다 작은 parameter/FLOPs와 낮은 학습 allocator peak
...
| 항목 | 값 | 판정 |
|---|---:|---|
| 데이터셋 | Adam Byerly, [micro-PCB Images](https://www.kaggle.com/datasets/frettapper/micropcb-images) | CC BY 4.0 |
| 사용 클래스 | `raspberry_pi_sbc` 1개 | CONFIRMED |
| 사용 이미지 | Raspberry Pi 1 B+, 3 B+, A+ 합계 1,875장 | CONFIRMED |
| train/val/test | 1,500/195/180 | CONFIRMED |
| condition/pHash component cross-split | 0 | 분할 검증 PASS |
| physical specimen independence | source ID 부족 | NOT VERIFIED |
| Ubuntu 새 카메라 테스트 | 미실행 | NOT VERIFIED |
...
|---|---|---|---|
| [`yolo11m_seed42_best.pt`](../../../weights/progress/rpi_bootstrap_2026-08-18/yolo11m_seed42_best.pt) | 추론 검토용 | COMPLETE | `f48e6e662190f0fd650ca55767cc4e32b18803a9ce3ebec240e51e2f54df5a28` |
| [`yolo11m_seed43_best.pt`](../../../weights/progress/rpi_bootstrap_2026-08-18/yolo11m_seed43_best.pt) | 추론 검토용 | COMPLETE | `901b8609ab93b4ed727912bf32f490acabbb397e20bef3ffd30e01b2f8de34ab` |
| [`yolox_s_seed42_best.pth`](../../../weights/progress/rpi_bootstrap_2026-08-18/yolox_s_seed42_best.pth) | 추론 검토용 | COMPLETE | `345ae7b7d54f49a447d3c6ab5c57d202c5593451a41678440c2ea72e94fb0538` |
...
- [비식별 처리한 원본 로그](yolo11m_seed42/terminal.log)
...
- [비식별 처리한 원본 로그](yolox_s_seed42/terminal.log)
...
이 gate를 충족하기 전까지 해당 파일은 `weights/trained`의 정식 모델이 아니라 재현·검토용 중간 공개
산출물입니다. 후속 epochs·seeds 목표를 변경할 경우에는 기존 증빙을 수정하지 않고 별도의 release
policy와 주장 범위를 동결해야 합니다.
