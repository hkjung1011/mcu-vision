...
def _scrub_checkpoint_value(value: Any, project_root: Path) -> Any:
    ...
    if isinstance(value, str):
        return _replace_local_path(value, project_root)
    return value
...
def _flatten_tensors(value: Any) -> list[Any]:
    try:
        import torch
    except ImportError as exc:  # pragma: no cover - exercised in the framework environment
...
def sanitize_yolo11_checkpoint(
    ...
    *,
    ...
    """Publish an Ultralytics checkpoint without machine-local path metadata.

    The tensor state is checked bit-for-bit and a deterministic zero-input forward pass is
    compared after reloading. The published checkpoint is intentionally a distinct artifact,
    so both the source and public SHA-256 are returned.
    """
    try:
        import torch
        from ultralytics import YOLO
    except ImportError as exc:  # pragma: no cover - exercised in the YOLO11 environment
    ...
    if maximum_absolute_difference != 0.0:
        raise ValueError(
            f"Published YOLO11 forward output changed: {maximum_absolute_difference}"
        )

    # Exercise the public loader, not only torch.load.
    YOLO(str(destination), task="detect")

    public_bytes = destination.read_bytes().lower()
    forbidden_spellings = {
        str(project_root.resolve()).encode("utf-8").lower(),
        str(project_root.resolve()).replace("\\", "/").encode("utf-8").lower(),
        str(Path.home().resolve()).encode("utf-8").lower(),
    }
    if any(spelling and spelling in public_bytes for spelling in forbidden_spellings):
        raise ValueError("Published YOLO11 checkpoint still contains a machine-local path")
